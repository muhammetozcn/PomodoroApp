...
export class SQLService {
  ...
  async init() {
    ...
    if (this.platform.is('android') || this.platform.is('ios')) {
      ...
      console.log('browser');
      const open = await window.openDatabase(DB_NAME, '1.0','DİSPLAY NAME', 5 * 1024 * 1024);
      this.db = webDB(open);
      console.log('DB KURULDU')
    }


    await this.storage.get('db').then(status => {
      console.log("Status belirleniyor");
      
      console.log("status:"+status)
     
      this.importSQL(DB_PATH);
    
     //status hem true dönüyor
    if (status) {
        console.log("db ready");
        this.dbReady.next(true);
    } else {
        this.importSQL(DB_PATH);
        console.log('İMPORTLANDI SQL')
      }
    });
  }
